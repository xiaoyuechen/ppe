...
xmlDocPtr
create_xml_stream (int width, int height, int quality, int window_size,
                   int block_size)
{
...
std::string
vector2str_ (float *mat, int size)
{
  std::string buf = "[ ";
  // char aux[20];
...
std::string
array2str_ (int *block_count, std::string **arr, int size)
{
  int i = 0;
  std::string buf;
...
void
stream_image (
    xmlDocPtr XML, xmlNodePtr parentNode, Channel *dc_diff,
    SMatrix *image_zero /*, int width, int height, int image_zero_size*/)
{
  xmlNodePtr dcNode = xmlNewNode (NULL, BAD_CAST "DC");
  std::string buf;
  std::string res = vector2str_ (
      dc_diff->data,
      dc_diff->height /*image_zero_size*/); // TO DO transpose matrix?
...
void
stream_frame (xmlDocPtr XML, int frame_number,
              std::vector<mVector> *motion_vectors, int ref_frame_number,
              Frame *dc_diff, FrameEncode *image_zero)
{
  // char buf[16];
  std::string buf;
  ...
  if (motion_vectors == NULL)
  ...
  else
    {
      ...
      buf = std::to_string (
          ref_frame_number); // sprintf(buf, "%d", ref_frame_number);
      xmlNewProp (frameNode, BAD_CAST "base", BAD_CAST buf.c_str ());
      xmlNodePtr motionVectorsNode
          = xmlNewNode (NULL, BAD_CAST "MOTION_VECTORS");
      for (int i = 0; i < (int)motion_vectors->size (); i++)
        {
          xmlNodePtr mvNode = xmlNewNode (NULL, BAD_CAST "MV");
          buf.clear (); // buf[0]='\0';
          // sprintf(buf,"[%d %d]", motion_vectors->at(i).a,
          // motion_vectors->at(i).b);
          buf = "[" + std::to_string (motion_vectors->at (i).a) + " "
                + std::to_string (motion_vectors->at (i).b) + "]";
...
void
write_stream (std::string stream_path, xmlDocPtr stream)
{
  xmlSaveFormatFileEnc (stream_path.c_str (), stream, "UTF-8", 1);
}

void
dump_matrix (float *matrix, int width, int height, const char *filename,
             int DOWNSAMPLED)
{
  if (!DUMP_TO_DEBUG)
    return;
...
void
dump_image (Image *image, const char *name, int frame_number)
{
  if (!DUMP_TO_DEBUG)
    return;
  ...
  dump_matrix (image->rc->data, image->rc->width, image->rc->height,
               rc_filename.c_str (), false);

  bc_filename.append (".Cb");
  bc_filename.append (".mat");
  printf ("Dumping %s\n", bc_filename.c_str ());

  dump_matrix (image->gc->data, image->gc->width, image->gc->height,
               bc_filename.c_str (), image->type);

  gc_filename.append (".Cr");
  gc_filename.append (".mat");
  printf ("Dumping %s\n", gc_filename.c_str ());

  dump_matrix (image->bc->data, image->bc->width, image->bc->height,
               gc_filename.c_str (), image->type);

  return;
}

void
dump_zigzag (Frame *frame, const char *name, int frame_number)
{
  if (!DUMP_TO_DEBUG)
    return;
  ...
  dump_matrix (frame->Y->data, frame->Y->width, frame->Y->height,
               Y_filename.c_str (), false);

  Cb_filename.append (".Cb");
  Cb_filename.append (".mat");
  printf ("Dumping %s\n", Cb_filename.c_str ());

  dump_matrix (frame->Cb->data, frame->Cb->width, frame->Cb->height,
               Cb_filename.c_str (), false);

  Cr_filename.append (".Cr");
  Cr_filename.append (".mat");
  printf ("Dumping %s\n", Cr_filename.c_str ());

  dump_matrix (frame->Cr->data, frame->Cr->width, frame->Cr->height,
               Cr_filename.c_str (), false);
}

void
dump_dc_diff (Frame *frame, const char *name, int frame_number)
{
  if (!DUMP_TO_DEBUG)
    return;
...
void
dump_frame (Frame *frame, const char *name, int frame_number)
{
  if (!DUMP_TO_DEBUG)
    return;
...
std::string function_name[10] = { "Convert to YCbCr:",
                                  "Low pass filter:",
